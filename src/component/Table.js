...
function Table() {
  ...
  function table1() {
    const bord = "1px solid beige";
    return (
      <tbody>
        {dataFilters &&
          dataFilters.map((dat, index) => (
            <tr key={index}>
              <td data-testid="planet-name" style={{ border: bord }}>
                {dat.name}
              </td>
  ...
  return (
    <table
      className="table"
      style={{ border: "2px solid beige", cellspacing: "20", width: "260" }}
    >
      <thead>
        <tr>
          <th>Name</th>
